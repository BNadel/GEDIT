...
DoCorrs = function(fname, outName){
  ...
  heatmap.2(as.matrix(flipped), trace = "none", dendrogram = "none", Rowv = FALSE, Colv = FALSE, col = brewer.pal(9,"Reds")[1:6], cellnote = round(flipped,2), notecol = "black", margins = c(8,12), main = "Errors By Tool, Reference, and Cell Type", rowsep = c(4,8,12,16,20), sepcol = "black", breaks = c(0.0,.05,.1,.15,.2,.25,.5))
  dev.off()}

DoCorrs("UnrolledCellMixtures.tsv", "CellMix")
