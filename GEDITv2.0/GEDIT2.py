...
def main():
  ...
  if outFile == None:
    outFile =  "_".join(stringParams) + "_Predictions.tsv"
    SaveLocation = curDir + "_".join(stringParams)
  else:
  ...
  MatrixTools.writeMatrix(Predictions,outFile)
  if SaveFiles:
     print "Deconvolution Complete: Results located in: " + SaveLocation
     print "for visualization run \'Rscript VisualizePredictions.R " + SaveLocation +  "\'"
     print
  else:
